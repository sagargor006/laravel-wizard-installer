...
@section('step')
    ...
    <form method="post" action="{{ route('install.database') }}">
        ...
        <div class="form-group">
            ...
            <input id="port" type="number" name="database_port" class="form-control" value="{{ isset($values['database_port']) ? $values['database_port'] : '3306' }}" required>
        </div>
        <div class="form-group">
            <label for="name">@lang('Database name') <span class="text-danger">*</span></label>
            <input id="name" type="text" name="database_name" class="form-control" value="{{ isset($values['database_name']) ? $values['database_name'] : 'laravel' }}" required>
        </div>
        <div class="form-group">
            <label for="username">@lang('Database username') <span class="text-danger">*</span></label>
            <input id="username" type="text" name="database_username" class="form-control" value="{{ isset($values['database_username']) ? $values['database_username'] : 'root' }}" required>
        </div>
        <div class="form-group">
